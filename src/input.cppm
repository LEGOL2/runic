...
public:
  bool IsKeyDown(Key key) const {
    ...
    return k < kKeyCount && current_keys_[k];
  }

  bool IsKeyPressed(Key key) const {
    auto k = static_cast<size_t>(key);
    return k < kKeyCount && current_keys_[k] && !previous_keys_[k];
  }
  ...
  bool IsMouseDown(MouseButton button) const {
    ...
    return b < kMouseButtonCount && current_mouse_buttons_[b];
  }

  bool IsMousePressed(MouseButton button) const {
    auto b = static_cast<size_t>(button);
    return b < kMouseButtonCount && current_mouse_buttons_[b] &&
           !previous_mouse_buttons_[b];
  }
  ...
  MousePosition GetMousePosition() const { return mouse_pos_; }
  ...
  void FlushFrameState() {
    ...
    mouse_delta_ = {0.0, 0.0};
  }

private:
  static constexpr size_t kKeyCount = GLFW_KEY_LAST + 1;
  static constexpr size_t kMouseButtonCount = GLFW_MOUSE_BUTTON_LAST + 1;
  ...
  MousePosition mouse_pos_{0.0, 0.0};
  MouseDelta mouse_delta_{0.0, 0.0};
};
} // namespace runic
